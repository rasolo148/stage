    ...
    <div id="wrapper">
      @include('template.SideBar')
        <div class="d-flex flex-column" id="content-wrapper">
            ...
            <header class="shadow-sm" style="width: 100%;height: auto;margin-top: -30px;padding: 0px;background-color: #ffffff;padding-left: 30px;padding-top: 14px;margin-bottom: 10px;"><label style="font-family: Nunito, sans-serif;font-size: 16px;">Stock des logistiques</label></header>
              ...
              <div class="row" style="margin-top:-80px;">
                <div class="col" style="margin-top: 100px;">
                    <div class="input-group" style="width: 100%;height: auto;padding: 8px;">
                        ...
                        <form action="{{ url('/recherchestock') }}" method="POST" class="d-none d-sm-inline-block mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search" style="width: 80%;margin: 0px;margin-right: 0px;margin-left: 60%;">
                          @csrf
                        ...
                        <table class="table table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <th  class="text-nowrap" style="background-color: #ffffff;">quantité restante</th>
                                </tr>
                            </thead>
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td  class="text-nowrap">{{ number_format($rows->prix_unitaire,2,',',' ') }}</td>
                                <td  class="text-nowrap">{{ $rows->quantite_restante }}</td>
                                <td  class="text-nowrap" style="width: 248px;">    
                                  <a href="{{ url('versmodifstock') }}/{{ $rows->libelle }}/{{ $rows->idlogistique }}/{{ $rows->idfournisseur }}/{{ $rows->prix_unitaire }}"><button class="btn btn-primary"  type="button" style="background-color: #eb3834;">Modifier</button></a>
                                </td>

                            </tr>
                            @if ($rows->quantite_restante == 0)
                            <tr>
                                <td  class="text-nowrap" colspan="5" style="color: red;">Attention: stock épuisé!</td>
                            </tr>
                            @endif
                              @endforeach
                    ...
                    <nav aria-label="Page navigation example">
                      <ul class="pagination">
                        <li class="page-item {{ $currentPage == 1 ? 'disabled' : '' }}">
                          <a class="page-link" href="{{ $currentPage == 1 ? '#' : url('/paginationstock') }}/{{ $currentPage - 1 }}" aria-label="Précédent">
                        ...
                        <li class="page-item {{ $rows == $currentPage ? 'active' : '' }}">
                          <a class="page-link" href="{{ url('/paginationstock') }}/{{ $rows }}">{{ $rows }}</a>
                        </li>
                        @endforeach
                        <li class="page-item {{ $currentPage == $lastPage ? 'disabled' : '' }}">
                          <a class="page-link" href="{{ $currentPage == $lastPage ? '#' : url('/paginationstock') }}/{{ $currentPage + 1 }}" aria-label="Suivant">
